...
def extract_design_info(registration_data: dict) -> dict:
    ...
    try:
        ...
        return {
            "allocation": design_info.get("allocationType") or design_info.get("allocation", ""),
...
def format_design_for_prompt(design: dict) -> str:
    ...
    if design.get("masking"):
        who = ", ".join(design["who_masked"]) if design.get("who_masked") else "not specified"
...
def extract_description(registration_data: dict) -> str:
    ...
    try:
        desc_module = registration_data.get("protocolSection", {}).get("descriptionModule", {})
        ...
        if detailed:
            parts.append("Detailed description (registry objectives and details):\n" + detailed)
        return "\n\n".join(parts)
    except Exception:
        return ""
...
def extract_participant_flow(registration_data: dict) -> str:
    ...
    try:
        flow_module = registration_data.get("resultsSection", {}).get("participantFlowModule", {})
        if not flow_module:
            return ""
        ...
        groups = {group["id"]: group["title"] for group in flow_module.get("groups", []) if "id" in group and "title" in group}
