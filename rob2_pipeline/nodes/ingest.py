...
def pdf_ingest_node(state: RoB2State) -> RoB2State:
    # Single PDF parsing path: docling. If docling fails (converter, chunking,
    # or structural extraction), the original exception propagates up so the
    # run halts visibly. Silent degradation to a different parser is worse
    # than a halt for diagnostic and research purposes.
    ...
    try:
        ...
        # LLM XML extraction is a separate concern from docling itself. If the
        # remote model fails or returns unparseable XML, fall back to the
        # structural extraction that docling already produced. This is not a
        # parser swap; it is a downstream-LLM-failure recovery using the same
        # docling output.
        evidence = extract_structural_paper_evidence(doc_repr)
