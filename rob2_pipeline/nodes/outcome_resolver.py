...
_PATTERNS = {
    "patient_reported": re.compile(r"\b(patient[- ]reported|self[- ]reported|questionnaire|quality of life|pain score|symptom score)\b", re.I),
    "safety_harm": re.compile(r"\b(adverse event|serious adverse|toxicity|harm|side effect|safety|tolerability)\b", re.I),
    "time_to_event": re.compile(r"\b(time to|survival|hazard ratio|kaplan[- ]meier|censor|event[- ]free)\b", re.I),
    "death_only": re.compile(r"\b(overall survival|all[- ]cause mortality|death from any cause|mortality|vital status)\b", re.I),
    "composite": re.compile(r"\b(composite|progression|relapse|recurrence|hospitali[sz]ation|treatment failure|event[- ]free|or death)\b", re.I),
    "lab_or_imaging": re.compile(r"\b(biomarker|laboratory|lab |blood|serum|imaging|radiographic|mri|ct scan|recist|threshold|assay)\b", re.I),
    "blinded_adjudication": re.compile(r"\b(blinded|masked|independent|central).{0,80}\b(adjudication|committee|review|assessor)\b", re.I),
}
...
def _death_is_sole_event(text: str) -> bool:
    ...
    return not re.search(r"\b(progression|relapse|recurrence|hospitali[sz]ation|treatment failure|composite|or death)\b", outcome_window, re.I)
...
def infer_outcome_properties(outcome: str, evidence_text: str) -> dict[str, bool]:
    ...
    props["composite"] = bool(_PATTERNS["composite"].search(text)) and not _death_is_sole_event(text)
    ...
    props["objective_event"] = _death_is_sole_event(text) or (
        props["lab_or_imaging_threshold"] and not props["patient_reported"] and not props["composite"]
    )
    props["clinician_judged"] = not props["patient_reported"] and not props["objective_event"]
...
def outcome_resolver_node(state: RoB2State) -> RoB2State:
    ...
    return {"outcome_properties": props, "outcome_type": resolved_type, "errors": errors}
