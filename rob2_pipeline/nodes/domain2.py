...
from rob2_pipeline.prompts import PROMPT_DOMAIN2_ANALYSIS, PROMPT_DOMAIN2_CONDITIONAL, PROMPT_DOMAIN2_SQ12
...
def domain2_conditional_node(state: RoB2State) -> RoB2State:
    sections = state["sections"]
    sq = state["sq_answers"]
    prompt = PROMPT_DOMAIN2_CONDITIONAL.format(
        intervention=state["intervention"],
        comparator=state["comparator"],
        outcome=state["outcome"],
...
def domain2_analysis_node(state: RoB2State) -> RoB2State:
    sections = state["sections"]
    prompt = PROMPT_DOMAIN2_ANALYSIS.format(
        intervention=state["intervention"],
        comparator=state["comparator"],
        outcome=state["outcome"],
...
def domain2_judge_node(state: RoB2State) -> RoB2State:
    judgment, rationale = judge_domain2(state["sq_answers"])
