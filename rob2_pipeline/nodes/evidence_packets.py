...
def _build_packet_for_contract(
    ...
    facts = [
        _source_to_fact(contract, source, confidence)
...
def _missing_evidence(
    ...
    if contract.needs_prespecification and not any(
        term in lowered for term in _PRESPEC_TERMS
    ):
...
def _negative_flags(
    ...
    flags: list[str] = []
    if contract.outcome_bound and _looks_like_wrong_outcome(
    ...
    if (
        contract.domain == "d5"
        and _RESULT_STAT_RE.search(text)
        and not any(term in lowered for term in _PRESPEC_TERMS)
    ):
