...
def test_domain1_keeps_structured_evidence_when_rag_context_exists(monkeypatch):
    ...
    def fake_call_node_llm(state, prompt, node_name, parse_fn, parse_sq_ids):
        ...
        parsed = {
            "1.1": {"answer": "Y", "quote": "central", "justification": "random", "uncertainty_flag": "NORMAL"},
            "1.2": {"answer": "Y", "quote": "central", "justification": "concealed", "uncertainty_flag": "NORMAL"},
            "1.3": {"answer": "N", "quote": "balanced", "justification": "balanced", "uncertainty_flag": "NORMAL"},
        }
        return "", [], parsed

    monkeypatch.setattr("rob2_pipeline.nodes.domain1.call_node_llm", fake_call_node_llm)
    evidence = empty_paper_evidence()
    evidence["d1_randomization"]["text"] = "Allocation managed by the ECOG-ACRIN Statistical Center."
    ...
    state = {
        ...
        "evidence": evidence,
        "rag_contexts": {"d1": "Patients were assigned to ADT alone or ADT plus docetaxel."},
        "sq_answers": {},
    }

    domain1_sq_node(state)

    assert "Allocation managed by the ECOG-ACRIN Statistical Center" in captured["prompt"]
